...
-(void)createBallsAndAnchors
{
    ...
    CGFloat ballSize = CGRectGetWidth(self.view.bounds)/(3.0*(ballCount-1));
    
    for(int i=0; i<ballCount; i++) {
        BallView *ball = [[BallView alloc] initWithFrame:CGRectMake(0, 0, ballSize-1, ballSize-1)];
        CGFloat x = CGRectGetWidth(self.view.bounds)/3.0+i*ballSize;
...
- (UIView *)createAnchorForBall:(BallView *)ball
{
    ...
    //根据球的位置确定描点的位置
    anchor.y -= CGRectGetHeight(self.view.bounds) / 4.0;
